MODULE dom_oce
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:) ::   mask_hdiff          !: LOLO: mask for horizontal tracer diffusivity !LOLO
...
CONTAINS
   ...
   INTEGER FUNCTION dom_oce_alloc()
      ...
      ALLOCATE( mbathy(jpi,jpj) , bathy(jpi,jpj) ,                                      &
         ...
         &     bmask(jpi,jpj)   , mask_hdiff(jpi,jpj)   ,                               & !LOLO
         &     mbkt   (jpi,jpj) , mbku (jpi,jpj) , mbkv(jpi,jpj) , STAT=ierr(9) )

      mask_hdiff(:,:) = 0.0_wp !LOLO
